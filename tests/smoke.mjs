...
assert.ok(sw.includes('gestalt-v0.033'),'service worker version mismatch');
...
assert.ok(js.includes("const VERSION='0.033'"),'app version must match release identity');
...
assert.equal(read('VERSION').trim(),'0.033','version file mismatch');
assert.equal(JSON.parse(read('package.json')).version,'0.0.33','package version mismatch');
assert.match(read('server/index.mjs'),/VERSION='0.033'/,'server version mismatch');
assert.match(read('sw.js'),/gestalt-v0\.033/,'service worker cache version mismatch');
