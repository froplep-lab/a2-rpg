...
assert.ok(words.every(w=>w.german&&w.ukrainian&&w.emoji&&w.sentence&&w.topicId&&w.sourcePage),'word record missing required study/source fields');
...
assert.equal(read('VERSION').trim(),'0.016','version file mismatch');
assert.equal(JSON.parse(read('package.json')).version,'0.0.16','package version mismatch');
