...
assert.match(css,/\.flash-back-visual\{[^}]*transform:rotateY\(180deg\)/,'back visual surface must keep its 180deg local orientation');
assert.match(css,/\.flash-back\{[^}]*transform:none/,'back translation content must stay in a non-transformed layer');
...
assert.match(css,/\.flash-face\{[^}]*transform-style:flat/,'front card face must stay flat inside the single 3D track');
assert.match(css,/\.flash-face\{[^}]*overflow:visible/,'card faces must not use overflow scrolling inside the 3D transform context');
assert.ok(css.includes('.flashcard.is-flipped .flashcard-inner{transform:rotateY(180deg)}'),'inner must own flip transform');
assert.match(css,/\.flashcard\.is-flipped \.flash-back\{opacity:1;visibility:visible;pointer-events:auto/,'back translation overlay must activate only after flip');
assert.ok(html.includes('class="flash-back-visual"'),'back visual surface missing');
