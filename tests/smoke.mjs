...
assert.ok(js.includes("const VERSION='0.032'"),'app version must match release identity');
...
assert.equal(read('VERSION').trim(),'0.032','version file mismatch');
assert.equal(JSON.parse(read('package.json')).version,'0.0.32','package version mismatch');
assert.match(read('server/index.mjs'),/VERSION='0.032'/,'server version mismatch');
assert.match(read('sw.js'),/gestalt-v0\.032/,'service worker cache version mismatch');
