...
const SAVE_KEY='gestalt_learning_v8';
const LEGACY_KEYS=['gestalt_learning_v7','gestalt_learning_v6','gestalt_learning_v5','gestalt_learning_v4','de_b1_rpg_progress_v3','deutsch_quest_v002'];
const TG_PREFIX='gestalt_v8_';

let tg=null, words=[], voiceList=[], telegramSaveTimer=null;
const $=id=>document.getElementById(id);
const state={screen:'learn',subview:'learn',mode:'learn',session:[],sessionIndex:0,seen:new Set(),flipped:false,answerLock:false,favorites:new Set(),toastTimer:null,lastSpokenKey:'',transitionTimer:null,currentTopic:'topic-8',swipeStartX:0,swipeStartY:0};
...
function answerWord(q){
  ...
  if(q===0){
    ...
    const intervalBox=clamp(box+1,1,5);
    m=Math.min(5,m+1);
    box=intervalBox;
    save.correct++;
  }
...
function navigate(screen){if(screen==='add'){openAdd();return}const changed=state.screen!==screen;state.screen=screen;if(screen==='learn'){state.mode='learn';state.subview='learn'}if(screen==='review'){state.mode='review';state.subview='learn'}if(screen==='collections'){state.subview='collection'}document.querySelectorAll('.screen').forEach(s=>s.classList.toggle('active',s.id===`screen-${screen==='collections'?'collections':screen==='settings'?'settings':screen==='review'?'review':'learn'}`));document.querySelectorAll('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===screen));$('pageTitle').textContent=screen==='learn'?'Слова':titles[screen]||'Слова';if((screen==='learn'||screen==='review')&&(changed||!state.session.length))pickSession(screen==='review'?'review':'learn',state.currentTopic);renderAll();syncTelegramBack()}
...
async function cloudLoad(){if(!tg?.CloudStorage)return null;for(const prefix of [TG_PREFIX,'gestalt_v7_']){try{const count=await new Promise((res,rej)=>tg.CloudStorage.getItem(`${prefix}count`,(e,v)=>e?rej(e):res(Number(v)||0)));if(!count)continue;const keys=Array.from({length:count},(_,i)=>`${prefix}${i}`);const vals=await new Promise((res,rej)=>tg.CloudStorage.getItems(keys,(e,v)=>e?rej(e):res(v||{})));let raw='';for(const k of keys)raw+=vals[k]||'';if(raw)return normalizeSave(JSON.parse(raw))}catch{}}return null}
...
function bind(){
 ...
 $('cardWrap').addEventListener('click',e=>{if(e.target.closest('button'))return;toggleFlip()});$('cardWrap').addEventListener('pointerdown',e=>{state.swipeStartX=e.clientX;state.swipeStartY=e.clientY});$('cardWrap').addEventListener('pointerup',e=>{const dx=e.clientX-state.swipeStartX,dy=e.clientY-state.swipeStartY;if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)&&!state.answerLock){dx>0?answerWord(4):answerWord(0)}});$('cardWrap').addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggleFlip()}});$('speakWord').addEventListener('click',e=>{e.stopPropagation();speak(currentWord()?.german)});$('speakSentence').addEventListener('click',e=>{e.stopPropagation();speak(currentWord()?.sentence)});$('favoriteBtn').addEventListener('click',toggleFavorite);$('rememberBtn').addEventListener('click',()=>answerWord(4));$('forgotBtn').addEventListener('click',()=>answerWord(0));
