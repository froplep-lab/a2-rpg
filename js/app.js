...
function installDevBridge(){
  ...
  window.__GESTALT_DEV__={
    ...
    getComputedCard:()=>{const c=$('flashcard'),i=$('flashcardInner'),f=document.querySelector('.flash-front'),v=document.querySelector('.flash-back-visual'),b=document.querySelector('.flash-back');const cs=getComputedStyle(c),is=getComputedStyle(i),fs=getComputedStyle(f),vs=getComputedStyle(v),bs=getComputedStyle(b);let netBackIdentity=null;try{const im=new DOMMatrix(is.transform==='none'?undefined:is.transform),vm=new DOMMatrix(vs.transform==='none'?undefined:vs.transform);const n=im.multiply(vm);netBackIdentity=Math.abs(n.a-1)<1e-3&&Math.abs(n.d-1)<1e-3&&Math.abs(n.b)<1e-3&&Math.abs(n.c)<1e-3;}catch{}return {cardTransform:cs.transform,innerTransform:is.transform,frontTransform:fs.transform,backVisualTransform:vs.transform,backContentTransform:bs.transform,backContentOpacity:bs.opacity,backContentVisibility:bs.visibility,netBackIdentity,cardTransition:is.transitionDuration,backFaceVisibility:vs.backfaceVisibility,webkitBackfaceVisibility:vs.webkitBackfaceVisibility,perspective:getComputedStyle(document.querySelector('.flip-wrap')).perspective,frontRect:f?.getBoundingClientRect().toJSON?.(),backVisualRect:v?.getBoundingClientRect().toJSON?.(),backContentRect:b?.getBoundingClientRect().toJSON?.(),cardRect:c?.getBoundingClientRect().toJSON?.()};},
