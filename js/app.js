...
const SAVE_KEY='gestalt_learning_v7';
const LEGACY_KEYS=['gestalt_learning_v6','gestalt_learning_v5','gestalt_learning_v4','de_b1_rpg_progress_v3','deutsch_quest_v002'];
const TG_PREFIX='gestalt_v7_';

let tg=null, words=[], voiceList=[], telegramSaveTimer=null;
const $=id=>document.getElementById(id);
const state={screen:'learn',subview:'learn',mode:'learn',session:[],sessionIndex:0,seen:new Set(),flipped:false,answerLock:false,favorites:new Set(),toastTimer:null,lastSpokenKey:'',transitionTimer:null};
...
function persist(){safeSet(SAVE_KEY,JSON.stringify(save));scheduleTelegramSave()}
...
function intervalForQuality(q,box){
  ...
  if(q===0) return 1;
  if(q===5) return 30;
  const nextBox=clamp(Number(box)||0,0,5);
  return ladder[nextBox];
}
function answerWord(q){
  ...
  save.mastery[k]=m;
  const days=intervalForQuality(q,box);
  save.review[k]={box,dueAt:now()+days*86400000,lastSeen:now(),lastQuality:q};
  save.answers++;
  save.learnedToday++;
  save.history[todayKey()]=(Number(save.history[todayKey()])||0)+1;
  touchActivity();
  save.record=Math.max(save.record,save.streak);
  save.xp+=q===0?1:(q===5?5:4);
  persist();
  scheduleTelegramSave();
  haptic(q===0?'error':'success');
  const intervalText=days===1?'завтра':`через ${days} дн.`;
  toast(q===0?'Повернемо це слово завтра.':`Добре. Наступне повторення ${intervalText} 🌱`);
  state.flipped=false;
...
function renderStats(){const all=mergedWords();const mastered=all.filter(w=>mastery(w)>=5).length;const learning=all.filter(w=>mastery(w)>0&&mastery(w)<5).length;const acc=save.answers?Math.round(save.correct/save.answers*100):0;$('statTotal').textContent=all.length;$('statMastered').textContent=mastered;$('statLearning').textContent=learning;$('statAccuracy').textContent=`${acc}%`;$('statDue').textContent=`${dueWords().length} до повторення`;const counts=[0,0,0,0,0,0];all.forEach(w=>counts[mastery(w)]++);$('masteryBars').innerHTML=counts.map((n,i)=>`<div class="mastery-row"><span>Рівень ${i}</span><div class="mastery-track"><div class="mastery-fill" style="width:${all.length?Math.round(n/all.length*100):0}%"></div></div><b>${n}</b></div>`).join('');const days=[];for(let i=6;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const k=dateKey(d);days.push({v:Number(save.history[k]||0),l:['Нд','Пн','Вт','Ср','Чт','Пт','Сб'][d.getDay()]})}const mx=Math.max(1,...days.map(x=>x.v));$('activityChart').innerHTML=days.map(x=>`<div class="chart-col"><div class="chart-bar" style="height:${Math.max(8,Math.round(x.v/mx*100))}%"></div><span class="chart-label">${x.l}</span></div>`).join('')}
...
function addWord(){
  ...
  const w={id:uid(),german:de,ukrainian:ua,hint:ua,grammar:'Власне слово',emoji,sentence:ex,sentenceUa:'Ваш власний приклад.',level:'A2/B1',category:'Мої слова',source:'GESTALT · Моя колекція',frequency:1};
...
async function initTelegram(){const ok=await tgLoadSdk();tg=ok?window.Telegram.WebApp:null;if(!tg)return;applyTelegram();const remote=await cloudLoad();if(remote&&((remote.answers+remote.learnedToday)>(save.answers+save.learnedToday)))save=remote;persist();renderAll();$('tgStatus').textContent='Telegram Mini App · синхронізація готова'}
