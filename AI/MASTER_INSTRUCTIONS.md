# AI DEVELOPER OPERATING MANUAL — GESTALT v0.030
...
- Flashcard architecture invariant: `.flashcard-inner` is the only flipped track; `.flash-front` is the front face at `rotateY(0deg)` and `.flash-back` is the real back face at `rotateY(180deg)` with `backface-visibility:hidden`. Keep this standard two-face relationship; do not add shell transforms or overlay workarounds.
